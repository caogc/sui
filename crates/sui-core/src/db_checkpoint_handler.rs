    ...
    async fn upload_db_checkpoint_to_object_store(&self) -> Result<()> {
        ...
        if let Some((epoch, db_path)) = next_db_checkpoint_to_copy {
            if self.prune_and_compact_before_upload {
                // Get the local filesystem path to where checkpoint is stored
                let local_path = self.path_to_filesystem(db_path)?;
                // Invoke pruning and compaction
                self.prune_and_compact(local_path, *epoch).await?;
            }
